...
import { presets, type ThemeName, fontMap, type FontName } from '../theme/presets';
...
export default function ThemePanel() {
  const { theme, setTheme, brandHsl, setBrand, font, setFont, size, setSize } = useTheme();
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content
          className={cn(
            ...
            'data-[state=open]:animate-[slideInFromRight_250ms_cubic-bezier(0.16,1,0.3,1)]',
        ...
        >
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className={cn(
                          ...
                          active ? 'bg-[hsl(var(--brand))] text-white' : 'bg-[hsl(var(--muted))] text-[hsl(var(--foreground))]'
                        )}>
                          {themeIcons[preset]}
                        </div>
                        <div>
                          <div className="font-semibold">{themeLabels[preset]}</div>
                          <div className="text-xs text-[hsl(var(--muted-foreground))]">
                            {preset === 'light' && 'Fondo claro, ideal para el día'}
                            {preset === 'dark' && 'Fondo oscuro, ideal para la noche'}
                            {preset === 'greenapple' && 'Verde menta, fresco y único'}
                          </div>
                        </div>
                      </div>
                      {active && (
          ...
          <div className="mb-6">
            ...
            <Select.Root value={font} onValueChange={(v: FontName) => setFont(v)}>
              <Select.Trigger 
                className={cn(
                  ...
                  <Select.Viewport className="p-1">
                    {Object.keys(fontMap).map((fontName) => (
                      ...
                      >
                        ...
                        <Select.ItemText style={{ fontFamily: fontMap[fontName as FontName] }}>
                          {fontName}
                        </Select.ItemText>
                      </Select.Item>
                    ))}
            ...
            <p className="text-xs text-[hsl(var(--muted-foreground))] mt-2">
              Vista previa con <span style={{ fontFamily: fontMap[font] }} className="font-semibold">
                {font}
              </span>
            </p>
          </div>
